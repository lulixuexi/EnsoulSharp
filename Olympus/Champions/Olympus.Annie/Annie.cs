...
namespace Olympus.Annie
{
    ...
    internal class MenuSettings
    {
        ...
        public class Keys
        {
            ...
            public static readonly MenuKeyBind harassToggle         = new MenuKeyBind("harassToggle", "Harass Key", System.Windows.Forms.Keys.H, KeyBindType.Toggle);
            public static readonly MenuKeyBind saveStunToggle       = new MenuKeyBind("saveStunToggle", "Save Stun Key", System.Windows.Forms.Keys.N, KeyBindType.Toggle);            public static readonly MenuKeyBind flashR               = new MenuKeyBind("flashR", "Flash + R Key", System.Windows.Forms.Keys.T, KeyBindType.Press);
        }
    }
    internal class Annie
    {
        ...
        public static void OnLoad()
        {
            ...
            W.SetSkillshot(0.50f, 200f, float.MaxValue, false, SkillshotType.Cone);

            E = new Spell(SpellSlot.E);

            R = new Spell(SpellSlot.R, 600f);
            R.SetSkillshot(0.20f, 100f, float.MaxValue, false, SkillshotType.Circle);
            ...
            if (MenuSettings.Combo.useR.Enabled && R.IsReady() && !objPlayer.HasBuff("AnnieRController") && target.IsValidTarget(R.Range))
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if (getTargetPrediction.Hitchance >= HitChance.High && getTargetPrediction.AoeTargetsHit.Count() >= MenuSettings.Combo.minEnemiesToR)
                        {
        ...
        private static void LaneClear()
        {
            ...
            if (MenuSettings.LaneClear.useQ.Enabled && Q.IsReady())
            {
                ...
                foreach (var min in minTarget)
                {
                    ...
                    if (MenuSettings.LaneClear.useQonlyLastHit && Q.GetDamage(min) >= min.Health)
                    {
                    ...
                    else if (!MenuSettings.LaneClear.useQonlyLastHit)
                    {
                        if (objPlayer.ManaPercent < MenuSettings.LaneClear.minMana.Value)
                            return;
        ...
        private static void OnDraw(EventArgs args)
        {
            ...
            if (MenuSettings.Drawing.drawFlashR.Enabled)
            {
                ...
                Drawing.DrawCircle(objPlayer.Position, R.Range + 600, System.Drawing.Color.DarkBlue);
            }
        }
        ...
        private static void KillSteal()
        {
            foreach (var target in GameObjects.EnemyHeroes.Where(target => target.IsValidTarget(Q.Range)))
            {
                ...
                if (MenuSettings.Misc.killstealIgnite && summonerIgnite.IsReady() && target.Health < objPlayer.GetSummonerSpellDamage(target, SummonerSpell.Ignite) && target.IsValidTarget(600))
                {
                    objPlayer.Spellbook.CastSpell(summonerIgnite, target);
                    return;
                }
        ...
        private static void AutoStack()
        {
            ...
            else if (!MenuSettings.Misc.autoStackOnlyOnSpawn.Enabled && MenuSettings.Misc.minManaAS.Value < objPlayer.ManaPercent && !objPlayer.InFountain())
            {
                ...
                if (E.IsReady() && MenuSettings.Misc.autoStackE)
                {
                    E.Cast();
                }
                else if (W.IsReady() && MenuSettings.Misc.autoStackW)
                {
                    W.Cast(objPlayer.Position);
                }
            }
